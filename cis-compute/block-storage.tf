locals {
  ...
  volumes_to_replicate = {for k, v in (var.storage_configuration != null ? (var.storage_configuration["block_volumes"] != null ? var.storage_configuration["block_volumes"] : {}) : {}) : k => v if v.replication_availability_domain != null} 
}
...
data "oci_identity_availability_domains" "bv_ads_replicas" {
  provider = oci.replication_region
...
resource "oci_core_volume" "these" {
  for_each = var.storage_configuration != null ? (var.storage_configuration["block_volumes"] != null ? var.storage_configuration["block_volumes"] : {}) : {}
    ...
    dynamic "block_volume_replicas" {
      for_each = each.value.replication_availability_domain != null ? [1] : []
      content {
        availability_domain = data.oci_identity_availability_domains.bv_ads_replicas[each.key].availability_domains[each.value.replication_availability_domain - 1].name
...
resource "oci_core_volume_backup_policy_assignment" "these" {
  for_each = local.volumes_with_backup_policies
    ...
    policy_id = local.oracle_backup_policies[each.value.backup_policy]
}
