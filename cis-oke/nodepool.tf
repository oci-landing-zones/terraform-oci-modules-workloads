...
resource "oci_containerengine_node_pool" "these" {
  ...
  lifecycle {
    ...
    precondition {
      ...
      error_message = "VALIDATION FAILURE in compartment id. One of the attributes compartment_id or default_compartment_id must be used when specifying an ocid in the cluster_id attribute."
    }
  }
  ...
  node_config_details {
    dynamic "placement_configs" {
      ...
      content {
        ...
        subnet_id               = length(regexall("^ocid1.*$", each.value.networking.workers_subnet_id)) > 0 ? each.value.networking.workers_subnet_id : var.network_dependency[each.value.networking.workers_subnet_id].id
    ...
    nsg_ids       = each.value.networking.workers_nsg_ids != null ? [for nsg in each.value.networking.workers_nsg_ids : (length(regexall("^ocid1.*$", nsg))) > 0 ? nsg : var.network_dependency[nsg].id] : []
  }
  ...
  ssh_public_key = each.value.node_config_details.ssh_public_key_path != null ? file(each.value.node_config_details.ssh_public_key_path) : file(var.workers_configuration.default_ssh_public_key_path)
}
