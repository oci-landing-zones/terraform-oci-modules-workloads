...
Worker-only deployments and external-cluster references are unsupported. For a
virtual pool, use `mode = "virtual-node-pool"`, a supported pod shape, and a nonempty
`pod_nsg_ids` list. Explicit virtual fault domains remain blocked by upstream.

For multiple clusters, see [the for_each caller example](../multiple-clusters).
