...
variable "instances_configuration" {
  ...
  type = object({
    ...
    instances = map(object({ # the instances to manage in this configuration.
      ...
      cluster_id       = optional(string) # the cluster id this instance should be added to.
      ...
      placement = optional(object({ # placement settings
        availability_domain  = optional(number) # the instance availability domain. Default is 1.
        fault_domain         = optional(number) # the instance fault domain. Default is 1.
      }))
      boot_volume = optional(object({ # boot volume settings
        ...
        size = optional(number) # boot volume size. Default is 50GB (minimum allowed by OCI).
        preserve_on_instance_deletion = optional(bool) # whether to preserve boot volume after deletion. Default is true.
        secure_boot = optional(bool) # prevents unauthorized boot loaders and operating systems from booting.
        measured_boot = optional(bool) # enhances boot security by taking and storing measurements of boot components, such as bootloaders, drivers, and operating systems. Bare metal instances do not support Measured Boot.
        trusted_platform_module = optional(bool) # used to securely store boot measurements.
        backup_policy = optional(string) # the Oracle managed backup policy. Valid values: "gold", "silver", "bronze". Default is "bronze".
      }))
      volumes_emulation_type = optional(string) # Emulation type for attached storage volumes. Valid values: "paravirtualized" (default for platform images), "scsi", "iscsi", "ide", "vfio". Module supported values for automated attachment: "paravirtualized", "iscsi".
      networking = optional(object({ # networking settings
        type                    = optional(string) # emulation type for the physical network interface card (NIC). Valid values: "paravirtualized" (default), "e1000", "vfio".
        ...
        skip_source_dest_check  = optional(bool) # whether the source/destination check is disabled on the primary VNIC. Default is false.
        ...
        secondary_vnics         = optional(map(object({
          ...
          skip_source_dest_check  = optional(bool) # whether the source/destination check is disabled on the VNIC. Default is false.
          nic_index               = optional(number) # the physical network interface card (NIC) the VNIC will use. Defaults to 0. Certain bare metal instance shapes have two active physical NICs (0 and 1).
      ...
      encryption = optional(object({ # encryption settings
        ...
        encrypt_in_transit_on_instance_create = optional(bool) # whether to enable in-transit encryption for the instance. Default is set by the underlying image. Applicable at instance creation time only.
        encrypt_in_transit_on_instance_update = optional(bool) # whether to enable in-transit encryption for the instance. Default is set by the underlying image. Applicable at instance update time only.
        encrypt_data_in_use = optional(bool) # whether the instance encrypts data in-use (in memory) while being processed. A.k.a confidential computing.
      }))
      flex_shape_settings = optional(object({ # flex shape settings
        memory = optional(number) # the instance memory for Flex shapes. Default is 16GB.
        ocpus  = optional(number)  # the instance ocpus number for Flex shapes. Default is 1.
      }))
      cloud_agent = optional(object({ # Cloud Agent settings
        disable_management = optional(bool) # whether the management plugins should be disabled. These plugins are enabled by default in the Compute service.
        disable_monitoring = optional(bool) # whether the monitoring plugins should be disabled. These plugins are enabled by default in the Compute service.
...
variable "storage_configuration" {
  ...
  type = object({
    ...
    block_volumes = optional(map(object({    # the block volumes to manage in this configuration.
      ...
      availability_domain = optional(number)  # the availability domain where to create the block volume.     
      volume_size         = optional(number) # the size of the block volume.
      vpus_per_gb         = optional(number)  # the number of vpus per gb. Values are 0(LOW), 10(BALANCE), 20(HIGH), 30-120(ULTRA HIGH)
      attach_to_instances = optional(list(object({ # map to where to attach the block volume.
        ...
        attachment_type = optional(string) # the block volume attachment type. Valid values: "paravirtualized" (default), "iscsi".
        read_only = optional(bool) # whether the attachment is "Read Only" or "Read/Write". Default is false, which means "Read/Write".
      })))
      encryption = optional(object({ # encryption settings
        kms_key_id              = optional(string) # the KMS key to assign as the master encryption key. default_kms_key_id is used if this is not defined.
        encrypt_in_transit      = optional(bool)  # whether the block volume should encrypt traffic. Works only with paravirtualized attachment type. Default is false.
      }))
    ...
    file_storage = optional(object({ # file storage settings.
      ...
      file_systems = map(object({     # the file systems.
        ...
        availability_domain = optional(number)  # the file system availability domain..   
        kms_key_id          = optional(string) # the KMS key to assign as the master encryption key. default_kms_key_id is used if this is not defined.
        replication         = optional(object({ # replication settings
          is_target         = optional(bool) # whether the file system is a replication target. Default is false
          ...
          interval_in_minutes = optional(number) # time interval (in minutes) between replication snapshots. Default is 60 minutes.
        })) 
      ...
      mount_targets = optional(map(object({ # the mount targets.
        ...
        availability_domain = optional(number) # the mount target availability domain.  
        ...
        exports = optional(list(object({
          ...
          options = optional(list(object({ # optional export options.
            ...
            access   = optional(string) # type of access grants. Valid values (case sensitive): READ_WRITE, READ_ONLY.
            identity = optional(string) # UID and GID remapped to. Valid values(case sensitive): ALL, ROOT, NONE.
            use_privileged_source_port = optional(bool)   # If true, accessing the file system through this export must connect from a privileged source port.
          })))
        })))
      })))
