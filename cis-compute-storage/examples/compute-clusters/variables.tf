...
variable "rdma_clusters_configuration" {
  ...
  type = object({
    ...
    clusters = map(object({                                   # the clusters to manage in this configuration.
      ...
      cluster_network_settings = optional(object({
        ...
        instance_pool = optional(object({                     # Cluster instance pool settings.
          ...
          size  = optional(number)                            # The number of instances in the instance pool. 
        }))
        networking = object({
          subnet_id = string                          # The subnet where the primary VNIC is created.
          ipv6_enable = optional(bool,false)          # Whether IPv6 is enabled for the primary VNIC.
          ...
          secondary_vnic_settings = optional(object({ # Secondary VNIC settings
            subnet_id = string                            # The subnet where the secondary VNIC is created.
...
variable "rdma_cluster_instances_configuration" {
  ...
  type = object({
    ...
    configurations = map(object({                         # the instance configurations to manage in this configuration.
      ...
      instance_details     = optional(object({
      ...
      template_instance_id = optional(string)       # the instance configuration
